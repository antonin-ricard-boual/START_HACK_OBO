...
function CityApp() {
  ...
  const startFireDrill = useCallback(() => {
    ...
    const burnFraction = count === 0 ? 0 : count === 1 ? 1 : count === 2 ? 0.75 : count === 3 ? 0.45 : count === 4 ? 0.2 : count === 5 ? 0.05 : 0
    const plan = allHeld.slice(0, Math.max(count <= 1 && allHeld.length > 0 ? 1 : 0, Math.round(allHeld.length * burnFraction)))
  ...
  useEffect(() => {
    ...
    Promise.all(
      [...fireDrill.burning].map(instId => {
        ...
        return sellStock(user.uid, instId, holding.shares)
      })
